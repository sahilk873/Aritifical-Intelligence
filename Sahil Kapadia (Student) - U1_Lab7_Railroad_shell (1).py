...
def make_graph(nodes = "rrNodes.txt", node_city = "rrNodeCity.txt", edges = "rrEdges.txt"):
   ...
   for city in node_city.readlines():
      x = city.strip().split(" ", 1)
      code = x[0]
      city = x[1]
      nodeToCity[code] = city
      cityToNode[city] = code
   ...
   for line in nodes.readlines():
      ...
      lat = l[1]
      long = l[2]
      nodeLoc[code] = (lat, long)

   for line in edges.readlines():
      l = line.strip().split(" ", 2)
      start = l[0]
      end = l[1]
      ecost = calc_edge_cost(nodeLoc[start][0], nodeLoc[start][1], nodeLoc[end][0], nodeLoc[end][1])
      edgeCost[(end, start)] = ecost
      edgeCost[(start, end)] = ecost
      if start in neighbors:
         neighbors[start].add(end)
      else: neighbors[start] = {end}
      if end in neighbors:
...
def generate_path(state, explored, graph):
   ...
   while explored[state] != "s":
      pathcost += graph[4][(explored[state], state)]
...
def draw_final_path(ROOT, canvas, path, graph, col='red'):
   ...
   for i in range(len(path)-1):
      x = graph[5][path[i]]
      y = graph[5][path[i+1]]
      drawLine(canvas, x[0], x[1], y[0], y[1], col)
...
def bi_bfs(start, goal, graph, col):
   ...
   frontier, explored = [], {start: "s"}
   frontier.append(start)
   gfrontier, gexplored = [], {goal: "s"}
   gfrontier.append(goal)
   while frontier or gfrontier:
      s = frontier.pop(0)
      g = gfrontier.pop(0)
      for a in graph[3][s]:  #graph[3] is neighbors
         if a in gexplored:
            explored[a] = s
            spath, scost = generate_path(a, explored, graph)
            gpath, gcost = generate_path(a, gexplored, graph)
            spath = spath[:len(spath)-1]
            gpath = gpath[::-1]
            path = spath + gpath
            cost = scost + gcost
            print(f'The number of explored nodes of Bi-BFS: {len(explored) + len(gexplored)}')
      ...
      for a in graph[3][g]:
         if a in explored:
            ...
            path = spath + gpath
            draw_final_path(ROOT, canvas, path, graph)
         if a not in gexplored:
            gexplored[a] = g
            gfrontier.append(a)
            drawLine(canvas, *graph[5][g], *graph[5][a], col)
         counter += 1
...
def a_star(start, goal, graph, col, heuristic=dist_heuristic):
    ...
    while frontier:
      ...
      if s[1] == goal:
         ...
         return path, cost
      neighbors  = graph[3][s[1]]
      for a in neighbors:
         pcost = explored[s[1]][0] + graph[4][(s[1], a)]
         if a not in explored or explored[a][0] > pcost:
            hval = heuristic(a, goal, graph)
            addpath = s[2]+[a]
            frontier.push((pcost + hval, a, addpath))
            explored[a] = (pcost, addpath)
...
def bi_a_star(start, goal, graph, col, ROOT, canvas, check, heuristic=dist_heuristic):
     
      cost = heuristic(start, goal, graph)
      frontier = HeapPriorityQueue()
      gfrontier = HeapPriorityQueue()
      frontier.push((cost, start))
      gfrontier.push((cost, goal))
      explored = {start: (0, [start])} 
      gexplored = {goal: (0, [goal])}
      fset, gset = {start}, {goal}
      counter = 0
      while frontier and gfrontier:

         state = frontier.pop()[1]
         fset -= {state}
         if state in gset:
            path = explored[state][1][:-1] + gexplored[state][1][::-1]
            cost = explored[state][0] + gexplored[state][0]
            if (check == 0):
               print(f'The number of explored nodes of Bi - A star: {len(explored) + len(gexplored)}')
               print(f'The whole path: {path}')
               print(f'The length of the whole path {len(path)}')
            draw_final_path(ROOT, canvas, path, graph)
            if check == 0:
               return path, cost
            if check != 0:
               return path, cost, len(explored) + len(gexplored), len(path)
         for neighbor in graph[3][state]:
            path_cost = explored[state][0] + graph[4][(state, neighbor)]
            if neighbor not in explored or explored[neighbor][0] > path_cost:
               hval = path_cost + heuristic(neighbor, goal, graph)
               frontier.push((hval, neighbor))
               fset.add(neighbor)
               explored[neighbor] = (path_cost, explored[state][1] + [neighbor])
               drawLine(canvas, *graph[5][state], *graph[5][neighbor], col)
         counter += 1
         state = gfrontier.pop()[1]
         gset -= {state}
         if state in fset:
            path = gexplored[state][1][:-1] + explored[state][1][::-1]
            cost = gexplored[state][0] + explored[state][0]
            if check == 0:
               print(f'The number of explored nodes of Bi - A star: {len(explored) + len(gexplored)}')
               print(f'The whole path: {path}')
               print(f'The length of the whole path {len(path)}') 
            ...
            if check != 0:
               return path, cost, len(explored) + len(gexplored), len(path)
         for neighbor in graph[3][state]:
            path_cost = gexplored[state][0] + graph[4][(state, neighbor)]
            if neighbor not in gexplored or gexplored[neighbor][0] > path_cost:
               hval = path_cost + heuristic(neighbor, start, graph)
               gfrontier.push((hval, neighbor))
               gset.add(neighbor)
               gexplored[neighbor] = (path_cost, gexplored[state][1] + [neighbor])
               drawLine(canvas, *graph[5][state], *graph[5][neighbor], col)
         counter += 1
         if counter % 1000 == 0: ROOT.update()
      return None
...
def tri_directional(start, goal, third, graph, col, ROOT, canvas, heuristic=dist_heuristic):
      path, cost, explored, pathlen = bi_a_star(start, third, graph, col, ROOT, canvas, 1)
      path2, cost2, explored2, pathlen2 = bi_a_star(third, goal, graph, col, ROOT, canvas, 1)
      final = path[::-1][:-1] + path2[::-1]
      #print(f'The number of explored nodes of Bi - A star: {explored + explored2}')
      print(f'The whole path: {final}')
      print(f'The length of the whole path {pathlen + pathlen2}') 
      draw_final_path(ROOT, canvas, final, graph)
      return final, cost + cost2
...
def main():
   ...
   draw_all_edges(ROOT, canvas, graph)
   cur_time = time.time()
   path, cost = bi_a_star(graph[2][start], graph[2][goal], graph, 'orange', ROOT, canvas, 0)
   if path != None: 
      draw_final_path(ROOT, canvas, path, graph)
