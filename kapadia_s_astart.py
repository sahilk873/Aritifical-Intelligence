...
def a_star(start, goal="_123456789ABCDEF", heuristic=dist_heuristic, size = 4):
   ...
   while frontier:
        ...
        if curstate[0] == goal:
            return curstate[2]
        for child in generate_children(curstate[0], size):
            hval = heuristic(child, goal, size) + len(curstate[2])
            hval += 1
            if not (child in explored):
                explored[child] = hval
                frontier.push((child, hval, curstate[2] + [child]))
            if(explored[child]>hval):
                explored[child] = hval
                frontier.push((child, hval, curstate[2] + [child]))
